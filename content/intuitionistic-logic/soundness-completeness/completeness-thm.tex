...
every $\Gamma \Proves/ !A$ an infinte tree model witnessing the 
fact that $\Gamma \Entails/ !A$. The following proposition shows 
...
\begin{prob}
Finish the proof of \olref{thm:decidability} by showing that $\mSat{M,w}{!A}$ iff 
$\mSat{M',[w]}{!A}$ for all formulas $!A$ with only propositional variables from $P$. 
\end{prob}

From \olref{thm:decidability} it follows that there is an algorithm to decide whether $\Entails !A$.
